...
# Runtime-configurable paths (defaults resolved in configure_paths()).
PATHS: Dict[str, Path] = {}


def _normalize_cli_path(raw_path: str) -> Path:
    """Resolve CLI-provided path with sensible relative-path behavior."""
    p = Path(raw_path).expanduser()
    if p.is_absolute():
        return p
    for base in [Path.cwd(), BASE_DIR, ANALYSIS_DIR]:
        cand = (base / p).resolve()
        if cand.exists():
            return cand
    # Return CWD-relative if it does not exist yet (e.g., output path).
    return (Path.cwd() / p).resolve()


def _resolve_input_path(arg_value: Optional[str],
                        default_candidates: List[Path],
                        required: bool,
                        key_name: str) -> Path:
    """Resolve a path from CLI or fallback candidates."""
    if arg_value:
        resolved = _normalize_cli_path(arg_value)
    else:
        existing = [p for p in default_candidates if p.exists()]
        resolved = existing[0] if existing else default_candidates[0]

    if required and not resolved.exists():
        raise FileNotFoundError(
            f"Missing required input for --{key_name.replace('_', '-')}: {resolved}"
        )
    return resolved


def configure_paths(args: argparse.Namespace) -> None:
    """Initialize runtime paths from CLI args or defaults."""
    files_dir = BASE_DIR / "files"
    code_dir = BASE_DIR / "code"

    PATHS["eval_json"] = _resolve_input_path(
        args.eval_json,
        [files_dir / "stage2_eval_set_detailed.json",
         BASE_DIR / "stage2_eval_set_detailed.json"],
        required=True,
        key_name="eval_json",
    )
    PATHS["test_json"] = _resolve_input_path(
        args.test_json,
        [files_dir / "stage2_test_set_detailed.json",
         BASE_DIR / "stage2_test_set_detailed.json"],
        required=True,
        key_name="test_json",
    )
    PATHS["task1_labels"] = _resolve_input_path(
        args.task1_labels,
        [files_dir / "task1_eval_labels.txt",
         BASE_DIR / "task1_eval_labels.txt"],
        required=True,
        key_name="task1_labels",
    )
    PATHS["task2_labels"] = _resolve_input_path(
        args.task2_labels,
        [files_dir / "task2_eval_labels.txt",
         BASE_DIR / "task2_eval_labels.txt"],
        required=True,
        key_name="task2_labels",
    )

    # Optional analysis inputs (sections skip gracefully if missing).
    PATHS["debate_json"] = _resolve_input_path(
        args.debate_json,
        [code_dir / "debate_ablation" / "test_set_detailed.json",
         BASE_DIR / "apex_debate_ablation" / "test_set_full.json"],
        required=False,
        key_name="debate_json",
    )
    PATHS["debate_metrics_json"] = _resolve_input_path(
        args.debate_metrics_json,
        [code_dir / "debate_ablation" / "debate_round_caps_metrics.json",
         BASE_DIR / "apex_debate_ablation" / "apex_debate_debate_round_caps_metrics.json"],
        required=False,
        key_name="debate_metrics_json",
    )
    PATHS["hu_json"] = _resolve_input_path(
        args.hu_json,
        [code_dir / "Hu_Debate_ablation" / "hu_debate_full.json",
         BASE_DIR / "Tianyu_Hu_Debate_ablation" / "debate_llm_judges_full.json"],
        required=False,
        key_name="hu_json",
    )
    PATHS["hu_metrics_json"] = _resolve_input_path(
        args.hu_metrics_json,
        [code_dir / "Hu_Debate_ablation" / "hu_debate_metrics.json",
         BASE_DIR / "Tianyu_Hu_Debate_ablation" / "debate_llm_judges_metrics.json"],
        required=False,
        key_name="hu_metrics_json",
    )

    if args.out_json:
        PATHS["out_json"] = _normalize_cli_path(args.out_json)
    else:
        PATHS["out_json"] = ANALYSIS_DIR / "results_ALL.json"
...
def load_detailed_json(path: Path) -> List[Dict[str, Any]]:
...
def section_b(test_data, eval_data, test_gold, eval_gold):
    ...
    for set_name, data, gold in [("EVAL", eval_data, eval_gold),
        ...
        if set_name == "TEST":
            ...
            # B8: Debate
            debate_metrics_path = PATHS["debate_metrics_json"]
            ...
            # B9: Hu debate
            hu_metrics_path = PATHS["hu_metrics_json"]
...
def section_d(test_data, test_gold):
    ...
    cap_metrics_path = PATHS["debate_metrics_json"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Comprehensive offline analysis for Stage-2 outputs."
    )
    parser.add_argument("--eval-json", default=None,
                        help="Path to stage2 eval detailed JSON.")
    parser.add_argument("--test-json", default=None,
                        help="Path to stage2 test detailed JSON.")
    parser.add_argument("--task1-labels", default=None,
                        help="Path to task1_eval_labels.txt.")
    parser.add_argument("--task2-labels", default=None,
                        help="Path to task2_eval_labels.txt.")
    parser.add_argument("--debate-json", default=None,
                        help="Optional path to debate detailed JSON.")
    parser.add_argument("--debate-metrics-json", default=None,
                        help="Optional path to debate round-caps metrics JSON.")
    parser.add_argument("--hu-json", default=None,
                        help="Optional path to Hu debate full JSON.")
    parser.add_argument("--hu-metrics-json", default=None,
                        help="Optional path to Hu debate metrics JSON.")
...
def main():
    ...
    print(f"  Test JSON: {PATHS['test_json']}")
    print(f"  Task1 labels: {PATHS['task1_labels']}")
    print(f"  Task2 labels: {PATHS['task2_labels']}")

    # Load data
    eval_data = load_detailed_json(PATHS["eval_json"])
    test_data = load_detailed_json(PATHS["test_json"])
